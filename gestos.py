...
# --- CONFIGURAÇÃO SERIAL ---
BAUD_RATE = 115200 
porta_serial = 'COM5' # Ajuste conforme seu gerenciador de dispositivos
ultimo_cmd = -1
...
def desenhar_checkbox(img, x, y, w, h, titulo, checked=False, cor_check=C_TEXTO):
    desenhar_texto_centralizado(img, titulo, x, y, w, 40, 0.6, C_TEXTO)
    bx, by, bs = x + (w-30)//2, y + 50, 30
    cv2.rectangle(img, (bx, by), (bx+bs, by+bs), C_TEXTO, 2)
    if checked: cv2.rectangle(img, (bx+4, by+4), (bx+bs-4, by+bs-4), cor_check, -1)
...
while cap.isOpened():
    ...
    # --- LÓGICA RESTRITA ---
    # Só indicador levantado (Anelar, Médio e Mindinho devem estar baixados)
    esq_ap = esq_st[1] and not any(esq_st[2:])
    dir_ap = dir_st[1] and not any(dir_st[2:])
    # Indicador e Médio levantados (Anelar e Mindinho baixados)
    ...
    if m_esq_ok and m_dir_ok:
        ...
        # Prioridades e Exclusividade
        if esq_ap and dir_ap and dist < 0.05: cmd = 2  # Together
        elif esq_ap and dir_ap: cmd = 1                # Duo
        elif esq_2d and dir_2d: cmd = 4                # Quad
        elif (esq_ap and dir_2d) or (esq_2d and dir_ap): cmd = 3 # Triple

    # Envio Serial Inteligente (só envia se houver mudança)
    ...
    if img_logo is not None:
        ...
        ly, lx = 10, 30
        if img_rsz.shape[2] == 4: # Blend Alpha para transparência
    ...
    desenhar_checkbox(canvas, 0, gy+2*hc, wc, hc, "Obj. Azuis", esq_2d, C_AZUL)
    desenhar_checkbox(canvas, wc, gy+2*hc, wc, hc, "Obj. Brancos", dir_2d, C_AMARELO)
